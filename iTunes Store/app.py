...
def search_for(query):
    ...
    args = {'term': query, 'country': country, 'media': 'macSoftware', 'limit': '10'}
...
def resultParse(item):
    ...
    itemID = item['trackId']
    directLink = "macappstore://phobos.apple.com/WebObjects/MZStore.woa/wa/viewSoftware?id=%s&mt=8" % (itemID)
    s = alp.Settings()
...
def iTunesSearch():
    ...
    results = []
    default_search = "macappstore://ax.search.itunes.apple.com/WebObjects/MZSearch.woa/wa/search?" + urllib.urlencode({"q": q})
    results.append(I(title="Search in App Store", subtitle="Open Mac App Store and search for \"{0}\"".format(q),
